...
export class InternAttendanceComponent implements OnInit, OnDestroy {
  ...
  // FIX: was 1, now 10 rows per page
  reportCurrentPage = 1;
  reportPageSize    = 10;
  readonly Math     = Math;
  ...
  // FIX: parse date parts with parseInt(..., 10) to avoid UTC timezone shift
  // that caused recordMonth to not match this.month, so green never showed
  async loadAttendance() {
    try {
      ...
      myRecords.forEach(record => {
        ...
        const recordMonth = parseInt(parts[1], 10) - 1; // convert to 0-indexed
        const recordDay   = parseInt(parts[2], 10);

        recordsByDate[record.date] = record;

        // Only populate calendar status for the currently viewed month+year
  ...
  async downloadDTRPdf() {
    ...
    try {
      ...
      // signature block renders with blank lines
    }
